...
class DatabaseManager {
  constructor(userDataPath) {
    ...
    const dbPath = path$1.join(userDataPath, "doc-embedder.db");
  ...
  getDashboardStats() {
    const projects = this.db.prepare("SELECT COUNT(*) as count FROM projects WHERE archived = 0").get();
    const documents = this.db.prepare("SELECT COUNT(*) as count FROM documents").get();
    const chunks = this.db.prepare("SELECT COUNT(*) as count FROM chunks").get();
    const vectorStores = this.db.prepare("SELECT COUNT(*) as count FROM vector_store_connections").get();
  ...
  // --- Documents ---
  addDocument(projectId, fileName, filePath, sourceType = "file") {
    const id = v4();
    const stmt = this.db.prepare(`
  ...
  _parseProject(row) {
    ...
    return {
      ...
      embedding_config: r.embedding_config ? JSON.parse(r.embedding_config) : null,
      chunking_config: r.chunking_config ? JSON.parse(r.chunking_config) : null,
      vector_store_config: r.vector_store_config ? JSON.parse(r.vector_store_config) : null,
...
class PostgresManager {
  ...
  async ensureTables(connectionString, config) {
  ...
  async insertVectorData(connectionString, config, doc, chunks, embeddings) {
    const client = new Client({ connectionString });
    try {
      await client.connect();
...
class OllamaManager {
  async testConnection(baseUrl) {
    ...
    try {
      ...
      return { success: true, version: ((_a = response.data) == null ? void 0 : _a.version) || "Unknown" };
    } catch (error) {
      return { success: false, error: error.message };
    }
  }
  async checkModel(baseUrl, modelName) {
    var _a;
    try {
      const response = await axios.get(`${baseUrl}/api/tags`);
      const models = ((_a = response.data) == null ? void 0 : _a.models) || [];
...
class OpenAIManager {
  ...
  async getEmbedding(apiKey, model, input) {
